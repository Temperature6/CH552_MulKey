...
#define ENABLE_SLEEP_MODE 1       //伪睡眠模式 - 当长按编码器使LED熄灭后，键盘功能也停止
...
#define KEY1_VAL_COUNT_G1 3
#define KEY2_VAL_COUNT_G1 3
#define KEY3_VAL_COUNT_G1 3
...
uint8_t CurCfg = 0;                  //当前配置，对于两组键值配置来说，0代表第一组，1代表第二组，该值的初始化在决定了上电后的键组
uint8_t LedState = 1;
uint8_t LongPress = 0;
unsigned long LPBeg = 0;
unsigned long EC_LastCall = 0;
unsigned long SW_LastCall = 0;
...
void loop() {
  ...
  if (digitalRead(EC11_D_PIN) == LOW && !LongPress)
  {
    ...
    if (digitalRead(EC11_D_PIN) == HIGH && millis() - SW_LastCall >= 500
#if ENABLE_SLEEP_MODE
      && LedState
#endif //ENABLE_SLEEP_MODE
    )
  ...
  if (LongPress && millis() - LPBeg >= LL_PRESS && millis() - EC_LastCall >= 2000)
  {
    LPBeg = millis();
    SW_LastCall = millis();
    LedState = !LedState;
    
    if (LedState) UpdateLed();
    else ClearLed();
  }
