...
__xdata uint8_t HIDMul[3] = { 2, 0x0, 0x0 };
...
uint8_t USB_EP1_send(uint8_t reportID) {
  ...
  if (reportID == 1) {
    ...
    Ep1Buffer[64 + 0] = 3;
    for (uint8_t i = 0; i < sizeof(HIDMul); i++) {  //load data for upload
      Ep1Buffer[64 + 1 + i] = ((uint8_t *)HIDMul)[i];
    }
    UEP1_T_LEN = 1 + sizeof(HIDMul);  //data length
  } else {
    UEP1_T_LEN = 0;
  }
...
uint8_t Keyboard_pressRaw(uint8_t k) {
  for (uint8_t i = 0; i < sizeof(HIDKey); i++) {  //load data for upload
    HIDKey[i] = 0;
  }
  HIDKey[2] = k;
...
uint8_t MulCtrl_press(uint8_t k) {
  //HIDMul[0] = 3;
  HIDMul[0] = k;
  USB_EP1_send(3);
}

uint8_t MulCtrl_releaseAll() {
  HIDMul[0] = 0;
  USB_EP1_send(3);
}
